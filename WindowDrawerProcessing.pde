...
void draw()
{
  if(start)
  {
    ...
    text("Lft Clk - Incr, Rt Clk - Decr", 30, height/4 -80);
    text("Robot 360 Time (ms): " + st, 30, height/4 -30);
...
void createCode(int[] pt)
{
  ...
  if(a > 0)
  {
    println("angleRight(" + round(a/ 360.0 * st) + ");");
  }
  else
  {
    println("angleLeft(" + round(-a/ 360.0 * st) + ");");
  }
  println("drive(" + d + ");");
  for(int c = 2; c <= pt.length/2 - 5; c+=2)
  {
    ...
    if(pt[c + 2] != 0 && pt[c + 3] != 0)
    {
      if(a > 180)
      {
        println("angleLeft(" + round((a - 180)/ 360.0 * st) + ");");
      }
      else if(a > 0)
      {
        println("angleRight(" + round(a/ 360.0 * st) + ");");
...
int drive(int p1, int p2, int p3, int p4)
{
  ...
  //println(dist * 100 / s);
  return round(dist * 1000 / (scale * s));
}
